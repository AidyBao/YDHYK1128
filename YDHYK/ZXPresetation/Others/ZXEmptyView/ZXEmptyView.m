...
+ (void)showNetworkErrorInView:(UIView *)view heightFix:(CGFloat)heightFix refreshAction:(ZXRCallBack)refreshAction{
    if (view) {
        ...
        [emptyView setText1WithText:@"访问服务失败"];
...
- (instancetype)init{
    if (self = [super init]) {
        [self setBackgroundColor:[UIColor zx_assistColor]];
...
- (void)setEmptyViewType:(HEmptyType)type{
    ...
    if (type == HEmptyTypeNoData) {
        [imgIcon setImage:[UIImage imageNamed:@"h_no_data"]];
        ...
        [imgIcon setImage:[UIImage imageNamed:@"h_network_error"]];
