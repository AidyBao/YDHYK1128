...
#define UM_KEY_APPSTORE         @"583e3b4307fe651cf1000088"
#define UM_KEY_ENTERPRISE       @"5872ec808f4a9d7485002122"
...
- (void)showAlertisAllowNotice:(BOOL)allow{
    if (!allow) {
        ...
        //访问被拒绝
        if (ZX_IOS8_OR_LATER) {
            [ZXAlertUtils showAAlertMessage:@"您阻止了程序接受消息,可能会错过提醒消息哦!" title:@"提示" buttonTexts:@[@"不再提示",@"马上打开"] buttonAction:^(int buttonIndex) {
                switch (buttonIndex) {
                    case 0:
                    {
            ...
            [ZXAlertUtils showAAlertMessage:@"您阻止了程序接受消息,可能会错过提醒消息哦!可在 '系统设置|通知' 中开启" title:@"提示" buttonTexts:@[@"不再提示",@"知道了"] buttonAction:^(int buttonIndex) {
                if (buttonIndex == 0) {
                    [[NSUserDefaults standardUserDefaults] setObject:@(1) forKey:@"ZX_Not_Show_Alert"];
                    [[NSUserDefaults standardUserDefaults] synchronize];
                }
